...
function generateChart() {
    ...
    activities.forEach((activity, index) => {
        ...
        const numDays = Math.floor((endDay - startDay) / (1000 * 60 * 60 * 24)) + 1; // Include the start day itself

        const taskDates = document.createElement('div');
        taskDates.classList.add('task-dates');
        taskDates.innerText = `${startDateWithTime} - ${endDateWithTime} (${numDays} days)`;
