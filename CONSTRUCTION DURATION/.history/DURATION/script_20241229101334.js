...
function generateChart() {
    ...
    activities.forEach((activity, index) => {
        ...
        taskName.classList.add(index % 2 === 0 ? 'pink-bg' : 'blue-bg');
        ...
        const adjustDays = document.createElement('div');
        adjustDays.classList.add('adjust-days');

        const decreaseBtn = document.createElement('button');
        decreaseBtn.innerText = '-';
        decreaseBtn.onclick = () => adjustTaskDays(index, -1);

        const increaseBtn = document.createElement('button');
        increaseBtn.innerText = '+';
        increaseBtn.onclick = () => adjustTaskDays(index, 1);

        adjustDays.appendChild(decreaseBtn);
        adjustDays.appendChild(increaseBtn);

        const taskDates = document.createElement('div');
        taskDates.classList.add('task-dates');
        taskDates.classList.add(index % 2 === 0 ? 'pink-bg' : 'blue-bg');
        taskDates.innerText = `${startDay.toLocaleDateString()} - ${endDay.toLocaleDateString()}`;

        taskContainer.appendChild(taskName);
        taskContainer.appendChild(bar);
        taskContainer.appendChild(adjustDays);
        taskContainer.appendChild(taskDates);

        chart.appendChild(taskContainer);
    });
}

function adjustTaskDays(index, adjustment) {
    activities[index].days += adjustment;
    if (activities[index].days < 1) {
        activities[index].days = 1; // Minimum duration is 1 day
    }
    generateChart();
}
...
window.onload = function() {
    ...
    generateChart(); // Generate chart for today's date by default
};
